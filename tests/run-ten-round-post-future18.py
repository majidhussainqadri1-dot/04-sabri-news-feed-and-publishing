...
need('Version: 2.0.6' in main and "SNFLA_VERSION', '2.0.6'" in main, 'Current runtime version must be 2.0.6 after the second audit.', fail)
need('Stable tag: 2.0.6' in wpreadme, 'Current WordPress stable tag must match runtime.', fail)
need('2.0.6' in readme and '2.0.6' in status, 'Current human release/status docs must identify 2.0.6.', fail)
need("VERSION='2.0.6'" in build and 'TEN_ROUND_REVIEW_ROUNDS=10' in build, 'Deterministic builder must retain first-ten-round evidence under v2.0.6.', fail)
...
if fail:
    ...
    sys.exit(1)
print('Historical first ten-round post-Future18 guardrails remain present under v2.0.6.')

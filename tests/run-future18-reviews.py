...
round1 = report('Future18 Review/Fix Round 1 — completeness, ownership, safety', [
    ...
    ('Version: 2.0.5' in main and 'SNFLA_Future18::boot' in main and 'SNFLA_Post_Audit_Hardening::boot' in main, 'Future18 plus second-audit v2.0.5 runtime must load and boot.'),
...
round2 = report('Future18 Review/Fix Round 2 — fresh adversarial release regression', [
    ...
    ("VERSION='2.0.5'" in build and 'FUTURE18_COUNT=18' in build and 'TEN_ROUND_REVIEW_ROUNDS=10' in build, 'Deterministic release builder must bind v2.0.5 to all 18 enhancements and ten-round evidence.'),
])

if not (round1 and round2): sys.exit(1)
print('Two consecutive fresh Future18 review/fix/retest rounds passed with the v2.0.5 second-audit hardening present.')

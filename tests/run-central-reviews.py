...
round1=report('Central-plan Review/Fix Round 1 — requirements, ownership and File 04 plan traceability',[
    ('Version: 2.0.2' in main, 'Runtime must be second-audit hardened patch 2.0.2.'),
    ...
    ('estimated_dispositions' in migration and 'storage_estimate' in migration and 'time_estimate' in migration and 'sample_diffs' in migration, 'File 04 dry-run must implement own-plan evidence fields.'),
...
round2=report('Central-plan Review/Fix Round 2 — fresh adversarial/source regression',[
    ...
    ('wp_insert_post(' not in central and '$wpdb->posts' not in central and 'wp_insert_post(' not in hard, 'Central/hardening layers must not write canonical publication storage.'),
    ...
    ("VERSION='2.0.2'" in build and 'central_plan_review_rounds' in build and 'TEN_ROUND_REVIEW_ROUNDS=10' in build, 'Deterministic package generator must identify v2.0.2 and all review evidence.'),
...
print('Two consecutive post-plan source reviews passed with v2.0.2 second-audit hardening and zero known blockers in the tested source scope.')

...
need('Version: 2.0.5' in main and "SNFLA_VERSION', '2.0.5'" in main,'Current runtime must be v2.0.5.',fail)
...
need("const INVALID_STATE = 'invalid'" in schema and 'snfla_lifecycle_state_invalid' in schema and ('SNFLA_Schema::state_valid()' in ret or 'in_array( $state, SNFLA_Schema::states(), true )' in ret),'Lifecycle invalid-state fail-closed control is missing.',fail)
...
need("VERSION='2.0.5'" in build and 'SECOND_TEN_ROUND_REVIEW_ROUNDS=10' in build,'Builder must bind v2.0.5 and second-ten-round evidence.',fail)

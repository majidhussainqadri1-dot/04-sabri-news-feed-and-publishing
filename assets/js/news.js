(function(){
  ...
  function request(data){
    ...
    return fetch(snpNews.ajaxUrl,{method:'POST',credentials:'same-origin',body:data}).then(function(response){return response.json().then(function(json){if(!response.ok||!json.success){throw new Error(json.data&&json.data.message?json.data.message:'The action could not be completed.');}return json.data;});});
  }
  ...
  document.addEventListener('DOMContentLoaded',function(){
    document.querySelectorAll('.snp-publish-form select[name="topic"]').forEach(function(select){
      ...
      function update(){var active=select.value==='patient-cases';box.hidden=!active;box.querySelectorAll('input[type="checkbox"]').forEach(function(input){input.required=active;if(!active){input.checked=false;}});}
